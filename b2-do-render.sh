...
if [ "$doStatus" = "active" ]; then
        ...
        ssh -o "StrictHostKeyChecking no" root@$doIP echo "'deb http://ftp.de.debian.org/debian jessie-backports main' >> /etc/apt/sources.list"
        ...
        ssh -o "StrictHostKeyChecking no" root@$doIP ffmpeg -i "bbb-4k.mp4" -threads 0 -vcodec libx264 -vf scale=1920x1080 -vb 7000K -acodec aac -ab 192k -ar 44100 -f mp4 "output.mp4"
        ssh -o "StrictHostKeyChecking no" root@$doIP b2 upload-file --threads 0 $b2Bucket  output.mp4 output.mp4
else
        echo "Failed"
fi
}
